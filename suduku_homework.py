...
class SudokuGUI(tk.Tk):
    def __init__(self):
        ...
        for i in range(9):
            for j in range(9):
                cell = tk.Label(self.sudoku_frame, text=" ",
    ...
    def create_sudoku_frame1(self, sudoku):
        for widget in self.sudoku_frame.winfo_children():
            widget.destroy()

        for i in range(9):
            for j in range(9):
                if  self.sudokus_to_solve[self.current_index][i][j] == 0 :
                    cell = tk.Label(self.sudoku_frame, text=sudoku[i][j] if sudoku[i][j] != 0 else " ",
                                    width=2, height=1, font=("Helvetica", 20), fg="red", relief="solid")
                    cell.grid(row=i, column=j)
                else:
                    cell = tk.Label(self.sudoku_frame, text=sudoku[i][j] if sudoku[i][j] != 0 else " ",
                                    width=2, height=1, font=("Helvetica", 20), relief="solid")
    ...
    def show_solution(self):
        if self.solved_sudokus:
            self.create_sudoku_frame1(self.solved_sudokus[self.current_index])
